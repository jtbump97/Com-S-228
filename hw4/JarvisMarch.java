...
public class JarvisMarch extends ConvexHull
{
	...
	public void constructHull()
	{
		...
		//initialize hullvertices to correct size
		hullVertices = new Point[rightChain.size()+leftChain.size()];
		
		int temp = leftChain.size();
		
		//add left side
		for(int j = 0; j< temp;j++) {
			
			hullVertices[j] = leftChain.pop();

		}

		//add right side
		for(int i = temp;i<hullVertices.length;i++) {
		...
		Point[] temp = new Point[hullVertices.length];
		int high = 0;
		int low = 0;
		int z = 0;
		for(int i = 0; i<hullVertices.length;i++) {
			...
			temp[i] = hullVertices[i];
			
		}
		
		
		//put vertices onto final array
		int flip = hullVertices.length-1;
		
		for(int x = 0;x<temp.length;x++) {
			
			hullVertices[x] = temp[flip];
			flip--;
		}
	...
	public void createLeftChain()
	{
		...
		//push the first point from this side onto list
		leftChain.push(highestPoint);
		
		//pushes all correct points onto list
		while (!leftChain.peek().equals(pointsNoDuplicate[0])) {
			
			leftChain.push(nextVertex(leftChain.peek()));	
			
		}
		//pops a redundant point
		leftChain.pop();
	}
	...
	public Point nextVertex(Point v)
	{
		...
		Point low = pointsNoDuplicate[0];
		
		for(int i = 0; i <pointsNoDuplicate.length;i++) {
				
			if(!pointsNoDuplicate[i].equals(v)) {
				
				if(comp.compare(pointsNoDuplicate[i], low)<0) {
					
					low = pointsNoDuplicate[i];
				}
			}
		}
