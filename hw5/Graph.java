...
public class Graph {
	 ...
	 *             if either of {@code fromIdx} and {@code toIdx} are invalid
	 *             vertex indices
	...
	public void addEdge(int fromIdx, int toIdx, int index, String word)
		...
		if (word != null) {
			...
			Vertex a = vertices[fromIdx];
			Vertex b = vertices[toIdx];

			Edge addA = new GraphEdge(index, word, vertices[fromIdx], vertices[toIdx]);
			Edge addB = new GraphEdge(index, word, vertices[toIdx], vertices[fromIdx]);
	...
	private class GraphEdge implements Edge {
		...
		public String toString() {
			...
			build.append("GraphEdge@").append(hashCode()) // address (unique per
															// instance)
