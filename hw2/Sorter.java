 ...
 * @throws IOException 
  ...
  public
  void
  sortWithStatistics(WordList toSort, Comparator<String> comp, int totalToSort)
    throws NullPointerException,
           IllegalArgumentException, IOException
  {
  
	  WordList temp=null;
	  int sorting =  1000000/toSort.length();
   ...
   long total = nextTime-startTime;
   totalSortingTime = totalSortingTime + total;
	 }
   CountingComparator<String> c = new CountingComparator<>(comp);
   temp = toSort.clone();
   this.sort(temp, c);
   totalComparisons = totalComparisons + c.getCount();
   
   
	
   String filename;
   filename = getClass().getSimpleName() + ".txt";
    ...
    implements Comparator<T>
  {
     ...
     */
    public
    CountingComparator(Comparator<? super T> wrapped)
      throws NullPointerException
    {
      if (null == wrapped)
      {
